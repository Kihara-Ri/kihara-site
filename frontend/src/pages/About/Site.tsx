...
这个项目在**工程形态**上是前后端分离的，但在**产品形态**上是一个一体化网站。

- **工程形态分离**：前端代码在 <code>frontend/</code>，后端代码在 <code>backend/</code>，本地开发时分别由 Vite 和 Go 服务运行。
- **产品形态一体**：前端只服务这一个网站，后端也只服务这一个网站，没有独立对外开放的通用 API 平台，也没有多客户端共用一套后端服务。
- **部署目标统一**：最终访问入口是同一个站点域名，只是线上通过 Nginx 做了反向代理。

所以更准确的说法不是“两个独立系统”，而是：**一个网站采用了前后端分离的开发方式。**
...
1. **本地开发**
- 根目录 <code>npm run dev</code> 会调用 <code>scripts/dev.sh</code>。
- 脚本同时启动 Go 后端和 Vite 开发服务器。
- Vite 负责前端热更新，Go 负责 API。
- 前端所有 <code>/api/*</code> 请求，通过 <code>VITE_API_PROXY_TARGET</code> 被转发到 Go 后端。

2. **当前线上部署**
- 前端先构建为静态文件。
- 远程服务器上的 Nginx 负责站点入口和反向代理。
- Go 服务负责处理 API。
- Nginx 把 API 请求转发到 Go；前端页面和静态资源则由 Nginx 自己提供。

当前线上结构可以概括为：

- **Nginx**：站点入口、HTTPS、静态文件、反向代理
- **Go**：API 服务
- **前端构建产物**：放在 Nginx 可访问目录
...
现在这个项目已经支持由 Go 直接托管前端构建产物，并且生产构建默认会把前端产物嵌入 Go 二进制，这也是后续简化线上部署的基础。

核心思路是：

1. 前端先执行 <code>vite build</code>，得到 <code>frontend/dist/</code>。
2. 构建脚本把这批文件同步到后端的嵌入资源目录。
3. Go 后端优先从嵌入文件系统提供静态资源。
3. 对于不是 <code>/api/*</code> 的请求：
- 如果命中了真实静态文件，就直接返回文件
- 如果没有命中，就回退到 <code>index.html</code>
4. 这样 React Router 的前端路由也能正常工作。

当前实现里：

- 后端构建时会把前端产物同步到嵌入资源目录并打进二进制
- 运行时也可以通过环境变量 <code>STATIC_DIR</code> 覆盖，优先读取外部静态目录
- 如果没有配置外部目录，Go 会直接使用嵌入资源
- 本地 Vite 联调依然不受影响，因为开发时前端入口仍然是 Vite 开发服务器

Go 侧最常见的实现方式有两种：

### 7.1 从磁盘读取构建目录

- 构建后把 <code>frontend/dist/</code> 放在服务器某个目录。
- Go 用 <code>http.FileServer</code> 或等价逻辑直接读取这批文件。
- Nginx 只需要把请求全部反代到 Go。
...
- 服务器上仍然要同时管理 Go 二进制和静态目录

### 7.2 直接打进 Go 二进制

- 先构建前端
- 再用 Go 的 <code>embed</code> 把 <code>frontend/dist</code> 编进后端
- 最终部署只需要一个 Go 二进制
...
- 部署最简单
- 服务器上不需要单独放前端目录
- Nginx 只做 HTTPS 和反向代理

缺点：

- 每次前端改动都要重新构建后端二进制

对于本站这种个人站，第二种通常更省心，因为最终部署单元会收敛成一个进程。
...
- 根目录也提供了统一检查和构建入口：<code>npm run check</code> 负责跑后端测试与前端类型检查，<code>npm run build</code> 会先构建前端，再输出后端二进制到 <code>release/</code>。
...
当前架构已经能稳定工作，但仍然有“前端产物要放到 Nginx 目录、前后端分别部署”的维护成本。更简化的演进路径是：
...
2. **线上使用 Go 托管前端静态资源**
- Nginx 只保留 HTTPS、gzip、缓存头和反向代理
- Go 同时负责 API 和前端静态文件
- 当前构建默认已经把前端结果嵌入 Go 二进制
...
- <code>deploy/nginx/kihara-site.conf</code>
- <code>deploy/systemd/kihara-site.service</code>
