...
const DATA_ROOT = "/projects/second-person-novel";
...
export function useNovelViewer() {
  ...
  async function loadGroup(groupMeta: PreviewGroupMeta) {
    ...
    const response = await fetch(`${DATA_ROOT}/${groupMeta.group_file}`);
    if (!response.ok) {
  ...
  useEffect(() => {
    ...
    async function boot() {
      try {
        setStatus("loading");
        setError(null);
        const response = await fetch(`${DATA_ROOT}/preview_index.json`);
        if (!response.ok) {
